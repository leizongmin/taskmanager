...
if (!mainFile)
  return die('Please specify a main file in envionment variable "RUNNER_MAIN".');
...
if (listenAddr) {
  ...
  if (isNaN(listen.port) || addr.length > 2 || !listen.host) {
    return die('Wrong listen address format in envionment variable "RUNNER_LISTEN": ' + listenAddr);
  }
}
else {
  var listen = null;
