...
exports.appDescription = function (config) {
  var execPath = config.execPath;
  var args = config.args.slice();
  if (!execPath)
    execPath = config.main;
  else
    args.unshift(config.main);
  return '命令：' + execPath + '，参数：' + args.join(' ');
};
...
exports.run = function (config, callback) {
  ...
  try {
    ...
    if (!execPath)
      ...
      args.unshift(config.main);
    var p = spawn(execPath, args, {env: config.env});
