...
var server = https.createServer(options, app.handler()).listen(config.port);
logger.info('Server listen at http://127.0.0.1:%d', config.port);
...
io.configure(function () {
  ...
  io.sockets.on('connection', function (socket) {
    ...
    socket.on('switch-app', function (id) {
      socket.appid = id;
    });
...
logger.on('line', function (line, method) {
  ...
  for (var i in io.sockets.sockets) {
    var c = io.sockets.sockets[i];
    if (appid !== false) {
      if (c.appid === appid) {
        c.emit('line', line, method);
      }
    } else {
      c.emit('line', line, method);
    }
